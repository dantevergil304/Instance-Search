...
def extract_feature_from_face_img_VGGFace2(face):
    face = cv2.resize(face, (224, 224))
...
def extractFeatVGGFace2(model, faces_path, frames_path, video_id):
    ...
    for batch, info in BatchGeneratorVGGFace2(faces_path, frames_path, video_id, batch_size=25):
...
if __name__ == '__main__':
    ...
    # start_t = time.time()
    # for video_id in range(70, 170):
    #     if os.path.exists(os.path.join(default_feature_folder, 'vggface2-SE-ResNet-50-256D', 'video' + str(video_id) + '.pkl')):
    #         print(video_id, 'already existed!')
    #         continue
